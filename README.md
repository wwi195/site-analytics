...
- **`raw`/`daily`シートの`date`列がDate型に化けて日付比較が全て失敗していた既知の不具合**:
  `setup()`で列を「プレーンテキスト」書式（`@`）にしていても、`appendRow()`など
  API経由で書き込んだ日付文字列（`"2026-09-20"`）は、Googleスプレッドシート側の
  自動判定によりDate型のセルとして保存されることがある。これにより
  `getTodayStats`/`getDashboardData`/`aggregateDaily`/`deleteOldRawRows_`の
  `String(row.date) === 対象日`という文字列比較が常に不一致となり、当日分PV/UUが
  0と表示されたり、日次集計・90日超データ削除が機能しなくなっていた。
...
- **運用初日はダッシュボードのサイト選択プルダウンが空になっていた既知の不具合**:
  ...
  グラフも描画されなかった。`getSiteList()` を `daily` と `raw` の両方から siteId を
  集めて重複除去するよう修正済み。`dashboard.gs` を修正した場合は、GASエディタに
  貼り付けて保存した後、「デプロイ」→「デプロイを管理」→ 既存デプロイの編集アイコン→
  バージョン「新バージョン」→デプロイ、で必ず再デプロイすること（URLは変わらない）。
